...
- Local app run command: `cd backend && uvicorn app.main:app --reload --port 8000`  
  (this evidence run used: `cd backend && python3 -m uvicorn app.main:app --host 127.0.0.1 --port 8000`)
...
- Test command: `cd backend && python3 -m pytest -v`
...
- No-baked-secrets check: **static PASS** — `backend/.dockerignore` excludes `.env` and `.env.*`; Dockerfile copies `requirements.txt` + `app/` only (no secret files observed in those COPY lines). Live image inspect **not confirmed**.
