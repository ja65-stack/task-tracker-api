...
- I will **never** paste a full local absolute path that includes my Windows
  ...
  such as `./` (repo root) or a placeholder like `C:\Users\<you>\...` instead.
...
1. Where must `pip install -r requirements.txt` and `pytest` run after the
   root-layout change, and what fails if that path is wrong?
...
(Python 3.11, `pytest -v` at repository root after layout flatten).

### Three lines to understand before committing

| Priority | Line(s) | Why first | Check question | I own this? |
|----------|---------|-----------|----------------|-------------|
| 1 | `pip install -r requirements.txt` | Install must find `requirements.txt` at repo root | If this ran in the wrong folder, what failure would you expect? | ☑ Yes |
...
| `pip install -r requirements.txt` | Installs app/test dependencies from repo root | Import/pytest failures |
| `pytest -v` | Runs the test suite; failures fail the job | False confidence if removed or masked |

### My answers

Date owned: 2026-07-29 (updated 2026-08-07 after moving out of `backend/`)

1. **requirements path:**  
   After the root-layout change, `requirements.txt` lives at the **repo root**.
   Running install from another directory would fail with a file-not-found style
   error. (Earlier, CI used `working-directory: backend` for the same reason.)
...
1. Install/tests must run where `requirements.txt` and `app/` live (repo root
   after the flatten; previously `backend/`).
...
|----------|------|--------------------------|------|
| Dockerfile | `Dockerfile` | ☑ Yes (multi-stage, `USER app`, no `--reload`) | 2026-07-29 |
| AGENTS.md | `AGENTS.md` | ☑ Yes (ban limited to `app/`, no destructive cmds) | 2026-07-29 |
| Security review | `docs/security-review.md` | ☑ Yes (S1–S7; open residual S1 / unverified S4) | 2026-07-29 |
| Activity / comments feature | `app/`, `frontend/index.html` | ☑ Yes (status PATCH omit-if-unchanged; activity after delete) | 2026-07-29 |

Per `AGENTS.md`: do not edit `app/` unless explicitly approved.
